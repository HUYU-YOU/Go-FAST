...
const uiFuel = document.getElementById('fuel'); // Nouvelle cible UI
...
// On injecte dynamiquement la mécanique de fuel sur le joueur
player.fuel = 100; 
...
function spawnEntities() {
    ...
    if(civilians.length < 15) {
        ...
        let cy = 100 + Math.random() * 800; // S'adapte aux nouveaux chemins
        civilians.push(new Civilian(cx, cy));
    }

    let maxPolice = cityLevel * 2;
    // Si le joueur est en panne sèche, on booste le spawn de la police pour mettre la pression !
    if(player.fuel <= 0) maxPolice += 2;

    if(police.length < maxPolice) {
        let px = player.x + (Math.random() > 0.5 ? 900 : -900);
...
function checkCollisions() {
    ...
    // Collecte de Fuel
    for(let f of map.fuels) {
        if(!f.collected && rectIntersect(pBounds, f)) {
            f.collected = true;
            player.fuel = Math.min(100, player.fuel + f.amount); // Recharge sans dépasser 100%
        }
    }
...
function update() {
    ...
    if (keys.up || keys.down || keys.left || keys.right) {
        // Consomme du fuel uniquement si le joueur appuie sur une touche de déplacement
        player.fuel -= 0.06; 
    }
    if(player.fuel < 0) player.fuel = 0;

    // Ajustement de la vitesse max selon le niveau de carburant
    if(player.fuel <= 0) {
        player.maxSpeed = 0; // Panne sèche : impossible d'accélérer
    } else {
        player.maxSpeed = 14; // Vitesse normale
    }
    ...
    // Affichage UI actualisé
    let cityLvl = Math.floor(player.x / map.cityWidth) + 1;
    uiHealth.innerText = `Dégâts : ${5 - player.health}/5`;
    
    // UI Fuel dynamique (change de couleur quand critique)
    uiFuel.innerText = `FUEL : ${Math.ceil(player.fuel)}%`;
    uiFuel.style.color = player.fuel < 25 ? '#ff0000' : '#ff5500';
...
function draw() {
    ...
    if(gameState !== 'playing' && msgScreen.style.display !== 'flex') {
        ...
        if(gameState === 'gameover_crash') {
            ...
            msgSub.innerText = player.fuel <= 0 ? "Panne de carburant... La police vous a encerclé !" : "Vous avez été arrêté par la police.";
        } else if (gameState === 'win') {
            msgTitle.innerText = "GO-FAST RÉUSSI !";
            msgTitle.style.color = "lime";
            msgSub.innerText = "Cargaison livrée avec le plein d'adrénaline.";
        }
    }
